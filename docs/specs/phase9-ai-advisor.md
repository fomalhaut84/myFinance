...
- [x] **9-A**: MCP 서버 구현 — myFinance 전용 도구 정의 + DB 조회 핸들러
- [x] **9-B**: Claude Code CLI 래퍼 + 시스템 프롬프트
- [x] **9-C**: 텔레그램 AI 질문 연동
- [ ] **9-D**: 웹 AI 분석 페이지
- [ ] **9-E**: 자연어 거래 입력 파싱
- [ ] **9-F**: 텔레그램 AI 응답 포맷 개선
...
- [ ] 시스템 프롬프트에 텔레그램 포맷 규칙 추가 (표 금지, 리스트/이모지 활용)
- [ ] 마크다운 → 텔레그램 HTML 변환 유틸 (`src/bot/utils/markdown.ts`)
- [ ] AI 응답을 `parse_mode: 'HTML'`로 전송 (볼드/이탤릭/코드 지원)
- [ ] 마크다운 표(table)를 텍스트 리스트로 변환
- [ ] 지원하지 않는 마크다운 태그 제거/변환

### 문제

텔레그램은 마크다운 표(`| --- |`)를 지원하지 않아 AI 응답이 그대로 텍스트로 노출됨.
`##` 헤더, `**볼드**`, 표 등이 가독성을 크게 떨어뜨림.

### 기술 설계

**1. 시스템 프롬프트 수정** — 텔레그램 전용 포맷 규칙:
```
텔레그램 응답 포맷:
- 표(table)를 절대 사용하지 않음
- 대신 리스트(-)와 들여쓰기로 정보 구조화
- 이모지를 적극 활용하여 시각적 구분
- 헤더는 볼드(**제목**)로 대체
- 구분선은 빈 줄로 대체
```

**2. 응답 후처리** — 마크다운 → 텔레그램 HTML 변환:
```
마크다운 표 → 리스트 변환
## 헤더 → <b>헤더</b>
**볼드** → <b>볼드</b>
...
**3. parse_mode 전환** — `ctx.reply(text)` → `ctx.reply(html, { parse_mode: 'HTML' })`

### 파일

```
src/lib/ai/system-prompt.ts       # 텔레그램 포맷 규칙 추가
src/bot/utils/markdown.ts         # (신규) 마크다운 → 텔레그램 HTML 변환
src/bot/commands/ai.ts            # parse_mode: 'HTML' 적용
```
