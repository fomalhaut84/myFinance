...
- [ ] **16-A**: 거래 CRUD API + 검증 유틸 + 후잉 웹훅 전송 유틸
- [ ] **16-B**: 거래 웹 UI + 네비게이션 개편 + 용어 정리 (Trade→종목 거래, Transaction→내역)
- [ ] **16-C**: 예산 API + UI (카테고리별 월 예산, 진행률, 월 복사)
- [ ] **16-D**: 가계부 분석 (전월 대비 증감, 지출 트렌드 3~6개월)
- [ ] **16-E**: 반복 거래 (RecurringTransaction 모델 + cron 자동 생성 + 관리 UI)
...
- [ ] **17-E**: 설정 통합 페이지 (계좌, 알림, 근로소득, 후잉 연동)