...
- [ ] **21-A**: 웹 자산 관리 페이지 (Asset CRUD UI — 추가/수정/삭제)
- [ ] **21-B**: 비주식 자산 증여 추적 (Deposit 모델을 Asset에도 연결, 아이별 입금/이체 기록)
- [ ] **21-C**: 아이별 통합 증여 현황 (주식 + 비주식 합산 뷰, 비과세 한도 대비)

## 기타 개선

- [ ] AI 주가 실시간 조회 (get_prices에서 fetchQuote 직접 호출)
