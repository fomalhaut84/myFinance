...
export default function StockOptionDeleteModal({ item, onClose, onDeleted }: StockOptionDeleteModalProps) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true)
    try {
      const res = await fetch(`/api/stock-options/${item.id}`, { method: 'DELETE' })
      ...
      <div className="fixed inset-0 flex items-center justify-center z-50 px-4">
        <div className="w-full max-w-[380px] bg-bg-raised border border-border rounded-[14px] overflow-hidden">
          <div className="px-6 py-5">
            <h2 className="text-[15px] font-bold text-bright mb-4">스톡옵션 삭제</h2>
            <div className="bg-card border border-border rounded-lg px-4 py-3 mb-4 flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[13px] text-bright font-medium">{item.displayName}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-dim">행사가격</span>
                <span className="text-[13px] text-bright tabular-nums">{formatKRW(item.strikePrice)}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[11px] text-dim">총부여수량</span>
                <span className="text-[13px] text-bright tabular-nums">{item.totalShares}주</span>
              </div>
            </div>
            <p className="text-[12px] text-red-400/80 leading-relaxed">삭제 시 행사 스케줄도 함께 삭제됩니다. 복구할 수 없습니다.</p>
