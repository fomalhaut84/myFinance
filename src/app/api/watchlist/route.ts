...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!body.market || typeof body.market !== 'string') return NextResponse.json({ error: '시장을 선택해주세요.' }, { status: 400 })
    ...
    const item = await prisma.watchlist.create({
      data: {
        ...
        targetBuy: typeof body.targetBuy === 'number' ? body.targetBuy : null,
        entryLow: typeof body.entryLow === 'number' ? body.entryLow : null,
        entryHigh: typeof body.entryHigh === 'number' ? body.entryHigh : null,
      },
    })
